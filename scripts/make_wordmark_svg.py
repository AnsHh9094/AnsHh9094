"""
Render "ANSH" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
def find_font():
    for path in FONT_CANDIDATES:
        if path and os.path.exists(path):
            idx = FONT_INDEX if path.endswith(".ttc") else 0
            return path, idx
    # Fallback: try loading by name (works on some systems)
    try:
...
def build_shell():
    ...
    pad_mask = np.pad(mask, 1)
    empty_r = ~pad_mask[1:-1, 2:]
    empty_l = ~pad_mask[1:-1, :-2]
    empty_d = ~pad_mask[2:, 1:-1]
    empty_u = ~pad_mask[:-2, 1:-1]
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">ansh@github: ~$ ./wordmark.sh --3d</text>')
    ...
    # intro: the resting pose wipes in left -> right behind a soft bar
